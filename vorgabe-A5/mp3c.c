...
void idTagFile(const char *fileName,char *comment)
{
	...
	if(comment != NULL){ //Falls comment == NULL schreib nichts, sonst schreib comment in file Kommentar berreich
		if(strlen(comment) > 30){
			printf("Kommentar hat eine maximale länge von 30\n");
			if(fclose(file) != 0){
		...
		//Kommentarbereich in File finden
		if(fseek(file, -31L, SEEK_END) != 0){
			//Fehler fseek
